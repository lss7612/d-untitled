개발·배포 팀을 활성화합니다. Backend / Frontend / DevOps 3명의 teammate를 spawn하세요.

## 지시사항

아래 순서대로 진행하세요:

1. 다음 3개 파일을 Read하여 각 teammate의 역할과 기술 스택을 파악하세요.
   - `.claude/commands/backend.md` — Backend Engineer
   - `.claude/commands/frontend.md` — Frontend Engineer
   - `.claude/commands/devops.md` — DevOps Engineer

2. 파일 내용을 숙지한 뒤, **3명의 teammate를 spawn하는 에이전트 팀을 생성**하세요.
   - 각 teammate의 생성 프롬프트에는 해당 파일의 역할·기술 스택·원칙을 포함하세요.
   - teammate 이름: `backend`, `frontend`, `devops`

3. 팀 생성 완료 후 사용자에게 팀 구성 현황을 보고하고 작업 요청을 기다리세요.

## 팀 운영 원칙

- 작업 요청이 들어오면 리더가 작업을 분석하여 적절한 teammate에게 할당하세요.
- 백엔드/프론트엔드는 기능 구현을 병렬로 진행하고, DevOps는 배포 및 인프라를 담당하세요.
- teammate들이 독립적으로 작업할 수 있도록 명확한 작업 경계를 설정하세요.
- 각 teammate의 결과물을 종합하여 최종 보고하세요.
