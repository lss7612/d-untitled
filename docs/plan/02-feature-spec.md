...
## 신규 모듈 참조 (implementation 문서)

초기 스펙 이후 실제 구현되며 파생된 세부 설계는 아래 문서에 나뉘어 기록한다.

- `08-book-catalog-exemption.md` — 책 카탈로그 + 클럽 단위 중복 체크 + 제한풀기.
- `09-owned-books-and-quota-exemption.md` — 보유책 페이지 마감 + 쿼터 기반 제한풀기(미구현).
- `10-club-membership-flow.md` — 가입 신청 / 관리자 승인 / 역할 변경.
- `11-developer-role-and-whitelist.md` — DEVELOPER 전역 역할 + 이메일 화이트리스트.
- `12-notification-abstraction.md` — NotificationService 추상화와 단계적 확장.
- `13-budget-module.md` — 월별 예산 + 회원간 나눔.
- `14-order-aladin-cart.md` — 신청 마감 → 합산 주문서 → 알라딘 카트 → 도착/수령.
- `15-book-report-feed.md` — 독후감 제출 + 동호회 피드 + 미제출 관리.
- `16-app-config.md` — 동적 설정 (AppConfig).
- `17-dev-tooling.md` — 더미 시드 · 진단 · 클린업.
